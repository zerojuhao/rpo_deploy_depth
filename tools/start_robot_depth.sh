...
# 函数：清理所有会话
cleanup_sessions() {
    screen -S inference_depth_session -X quit 2>/dev/null
...
verify_dds_effectiveness() {
    ...
    for node in "depth_node" "inference_node" "joy_node"; do
...
start_component "inference_depth_session" "ros2 launch inference inference_depth.launch.py" "inference_node" 5
if ! ros2 node list | grep -q "depth_node"; then
    print_error "inference_depth_session 启动失败！未检测到 depth_node 节点。"
    cleanup_sessions
    exit 1
fi
...
# 所有组件启动完成
print_success "----------------------------------------"
print_success "深度图感知行走组件已在后台成功启动！"
print_success "使用以下命令查看各组件输出："
print_success "深度感知推理模块: screen -r inference_depth_session"
print_success "手柄控制: screen -r joy_session"
print_success "----------------------------------------"
print_info "若要退出某个screen会话，按Ctrl+A然后按D"
print_info "使用以下命令停止所有组件："
print_info "screen -S inference_depth_session -X quit"
